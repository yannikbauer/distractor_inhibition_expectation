...
for iSub = doSubs        
    ...
    % Subject directories and file IDs
    ...
    elseif ~exist(fullfile(dirs.sub,fid.ft_prep.out), 'file') 
        ...
        cfg.detrend     = 'yes';
        cfg.demean      = 'yes';
        dat = ft_preprocessing(cfg); % do pre-preprocessing
        % NB: error when loading .set file, due to header referring to wrong file esXX_XX_filt_fixPost1000.fdt
        % wrong info is in dat.hdr.orig.filename/setname/datfile (see corresponding .mat file). To
        % change this, check fileRenamer.m
    ...
    % Get individual alpha peak power in a power-spectrum
    
    % Loop through all contrasts of interest (coi) to get power contrast contra>ipsi
...
for iTF = 1:length(coi.namesCombi)
    subplot(2,3,iTF);
    imagesc(tfdat.time-1, tfdat.freq, meanPwr4.(coi.namesCombi{iTF}));
%     set(gca,'Clim',cLims);
    set(gca,'Xlim',xLims);
    set(gca,'Ylim', yLims);
    axis xy; % invert axes
